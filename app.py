...
<style>
    ...
    div[data-testid="stFileUploader"] {
        border: 2px dashed #00A2E9; background-color: #E6F6FE; border-radius: 10px; padding: 20px;
    }
    .result-box {
        background-color: white; padding: 20px; border-left: 6px solid #ED1C24; 
        border-radius: 8px; box-shadow: 0px 4px 12px rgba(0,0,0,0.1); 
        white-space: pre-wrap; font-family: monospace; color: #333333; font-size: 15px; line-height: 1.6;
    }
    div[data-testid="metric-container"] {
        background-color: white; border: 1px solid #e0e0e0; border-radius: 8px; padding: 10px 15px;
        box-shadow: 2px 2px 10px rgba(0,0,0,0.05); border-top: 3px solid #FFCC00;
    }
    ...
    .stTabs [aria-selected="true"] { background-color: #00A2E9; color: white; border-color: #00A2E9; }
</style>
...
# 4. Area Upload (Support XLS, XLSX, CSV)
...
if uploaded_file is not None:
    try:
        ...
        # === GLOBAL VARIABLES (Untuk Tab Output Akhir) ===
        ...
        # === DETAILED TRACKER (Untuk Tab Detail Rincian) ===
        ...
        # Tambahan ke Dictionary Detail PB
        ...
        # --- UI INTERAKTIF TABS ---
        tab1, tab2, tab3 = st.tabs(["📊 Dashboard Visual", "🔍 Detail Rincian (List)", "📝 Rekap Akhir"])
        
        with tab1:
            st.markdown("### Ringkasan Cepat")
            met1, met2, met3, met4 = st.columns(4)
            met1.metric("Total Data Diproses", len(df))
            met2.metric("Total MCB 1PH Pasang", psg_mcb_1ph)
            met3.metric("Total LPB 1PH Pasang", psg_lpb_1ph)
            met4.metric("Kabel SR 1PH (Meter)", sr_1ph)
            
            st.markdown("### Grafik Perbandingan Pasang vs Bongkar")
